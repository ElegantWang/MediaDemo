...
public class MediaDecoderDemo {
    private static final String TAG = "MediaDecoderDemo";
    private static final boolean VERBOSE = true;
    private static final long DEFAULT_TIMEOUT_US = 10000;

    public static final int COLOR_FormatI420 = 1; // YUV420P  YYYYYYY UU VV
    public static final int COLOR_FormatNV21 = 2; // YUV420SP YYYYYYY VU VU
    ...
    public void decodeVideo(String videoFilePath, DecoderListener listener) throws IOException {
        ...
        try {
            ...
            decodeFramesToImage(decoder, extractor, trackIndex, mediaFormat, listener);
            decoder.stop();
        } finally {
            if (decoder != null) {
                decoder.stop();
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    private void decodeFramesToImage(MediaCodec decoder, MediaExtractor extractor, int trackIndex, MediaFormat mediaFormat, DecoderListener listener) {
        ...
        if (listener != null) {
            listener.onSize(this, width, height);
        }
        ...
        while (!outputDone) {
            ...
            if (!outputDone) {
                int decoderStatus = decoder.dequeueOutputBuffer(info, DEFAULT_TIMEOUT_US);
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    int outputBufferId = decoderStatus;
                    if (doRender) {
                        if (VERBOSE) Log.d(TAG, "awaiting decode of frame " + outputFrameCount);
                        Image image = decoder.getOutputImage(outputBufferId);
                        if (listener != null) {
                            listener.onFrameData(this, outputFrameCount, info.presentationTimeUs, getDataFromImage(image, colorFormat));
                        }
                        image.close();
            ...
            if (outputBufferId >= 0) {
                ...
                if (doRender) {
                    ...
                    *//*if (outputImageFileType != -1) {
                    ...
                    if (listener != null) {
                        listener.onFrameData(this, outputFrameCount, getDataFromImage(image, colorFormat));
                    }
                    image.close();
                    decoder.releaseOutputBuffer(outputBufferId, true);
                    outputFrameCount++;
    ...
    public interface DecoderListener {
        void onSize(MediaDecoderDemo decoderDemo, int width, int height);

        void onFrameData(MediaDecoderDemo decoderDemo, int frameSeq, long presentationTimeUs, byte[] data);
