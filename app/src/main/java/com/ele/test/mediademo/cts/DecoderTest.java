package com.ele.test.mediademo.cts;
...
import com.ele.test.mediademo.R;
...
public class DecoderTest {
    ...
    protected void setUp() throws Exception {
        ...
        AssetFileDescriptor masterFd = mResources.openRawResourceFd(R.raw.dvr_v_p479_4475272588);
            ...
            if (res >= 0) {
                ...
                if (info.size > 0) {
                    ...
                    if (isAudio) {
                        ...
                        long sum = checksum(codecOutputBuffers[res], info.size);
                        if (numframes < checksums.length) {
                            // assertEquals("frame data mismatch at frame " + numframes, checksums[numframes], sum);
                        }
                        numframes++;
                    }
                }
...
//                        "/" + numframes + "/" + info.flags);
    ...
    private void testFlush(int resource) throws Exception {
        ...
        extractor.seekTo(8000000, MediaExtractor.SEEK_TO_PREVIOUS_SYNC);
        codec.flush();
